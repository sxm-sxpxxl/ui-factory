...
### Internal
- The README describes the package as it is today: the handle-and-description model, the components, how cached meshes are reused through record equality and `Snapshot<T>` versions, a minimal control that draws with it, and the install steps under the `com.sxm-tools.ui-factory` name; the manifest description says the same in one sentence, and the preview image is drawn by the package itself: every line and marker in it is a mesh the factory built
