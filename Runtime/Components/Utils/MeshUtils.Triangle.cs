...
namespace SxmTools.UIFactory.Components
{
    ...
    {
        private static readonly Vector2[] VerticesOnTriangle = new Vector2[3];
        ...
        {
            BurstProcedures.FillEquilateralTriangle(ref data.Vertices, ref data.Indices, size, origin, angleAroundOriginInDeg, color);
        }

        public static Vector2[] GetVerticesOnEquilateralTriangle(float angleAroundOriginInDeg, float size, Vector2 origin = default)
        {
            var nativeVertices = VerticesOnTriangle.WrapAsNativeArray();
            BurstProcedures.FillEquilateralTriangle(ref nativeVertices, size, origin, angleAroundOriginInDeg);

            return VerticesOnTriangle;
        }
